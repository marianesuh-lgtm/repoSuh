...
public class OllamaTestService {
    ...
    public PagedStoryResponse generatePagedStory(GenerateBookRequest request, CharacterDTO character) {
...
//                .build());
    ...
    private String buildSystemPrompt(CharacterDTO character, int pageCount) {
        return """
                ...
                [규칙]
                1. 모든 답변은 100%% 순수 한국어(한글)로 작성합니다. (이미지 프롬프트 항목 제외)
				2. 첫 줄은 반드시 "제목: [귀엽고 창의적인 제목]" 형식으로 작성합니다.
				3. 정확히 %d페이지로 구성하며, 각 페이지는 2~3문장의 리듬감 있는 문체로 작성합니다.
				4. 각 페이지 하단의 영어 프롬프트(BG, ACTION, MOOD)는 다음 원칙을 따릅니다:
				   4.1. 모든 ACTION에는 반드시 '주인공'과 '부캐릭터'가 함께 등장해야 합니다. (예: A cat and a sparrow...)
				   4.2. 두 캐릭터의 물리적 관계를 명시하세요. (예: side by side, facing each other, bird on cat's shoulder)
				   4.3. 주인공의 고정 특징(%s)을 프롬프트 맨 앞에 배치하세요.
